...
export default function Shop({ user }: { user: TUser | null }) {
  ...
  useEffect(() => {
    Promise.all([getTop(), getProducts(), getCategories()]).then(
      ([retrievedTop, retrievedProducts, retrievedCategories]) => {
    ...
    <>
      <div className="fontRaleway flex flex-row justify-around w-screen mt-16 mb-16 max-sm:flex max-sm:flex-col max-sm:items-center max-md:flex max-md:flex-col max-md:items-center max-lg:flex max-lg:flex-col max-lg:gap-3 max-lg:items-center max-xl:flex max-xl:flex-row max-xl:gap-3 max-xl:items-center ">
        <div className="max-sm:w-full w-2/5 h-auto bg-gradient-linear3 rounded-3xl flex items-center gap-2 p-4 flex-col">
          <div className="flex w-full border-b-[1px] justify-between items-center py-4">
            <h4 className="font-semibold text-[28px] text-white  border-white    ">
              Ваши Продукты
            </h4>

            <button
              className="fontInter hover:bg-stone-200   bg-white text-black rounded-3xl mb-1 mt-1  py-2 px-4 text-sm max-sm:w-28 "
              type={'button'}
              onClick={() => {
          ...
          {products.length !== 0 ? (
            ...
            <p className="font-light text-lg text-white  border-white p-4 w-full  h-[75px]">
            ...
            {topShop?.data.map(top => {
              return (
                <div
                  ...
                  <div>
                    <p className="font-medium text-xl">
                      {top.profile.rating} ules points
                    </p>
                  </div>
                </div>
              );
      ...
      <div
        className="flex justify-center items-center py-6 absolute z-10 top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] w-[100vw] h-screen overflow-x-hidden scrollHide drop-shadow-2xl"
        style={{
            ...
            <div className="relative bg-white rounded-lg  ">
              <div className="px-6 py-6 lg:px-8">
                ...
                <form
                  ...
                  onSubmit={async event => {
                    ...
                    await createProduct({
                      ...
                      quantity_per_unit: Number(
                        quantityPerUnitRef.current.value
                      ),
                      units_on_order: 0,
                      discontinued: false,
                    }).then(() => {
                  ...
                  <button
                    type="submit"
                    className="w-full text-white bg-ulsDark hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                    Отправить
                  </button>
                </form>
              </div>
